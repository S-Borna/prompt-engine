...
export function EnhancedPromptOutput({
    ...
    return (
        <div className="flex flex-col gap-3 h-full">
            ...
            {/* ═══════════════════════════════════════════════════════════════
                MAIN CONTENT - Enhanced Prompt + Side Panel
            ═══════════════════════════════════════════════════════════════ */}
            <div className="flex-1 flex gap-3">
                ...
                {/* SIDE PANEL — Original Prompt & Explanation */}
                {animationDone && (result.originalPrompt || (result.explanation && result.explanation.length > 0)) && (
                        ...
                        {result.originalPrompt && (
                            <div className="rounded-xl border border-white/[0.04] overflow-hidden">
                                <button
                                    ...
                                    className="w-full flex items-center justify-between px-3 py-2.5 bg-white/[0.02] hover:bg-white/[0.03] transition-colors"
                                >
                                    <span className="text-[10px] text-white/40 uppercase tracking-wider font-medium">Original</span>
                                    <ChevronDown className={`w-3.5 h-3.5 text-white/30 transition-transform ${showOriginal ? 'rotate-180' : ''}`} />
                                </button>
                                {showOriginal && (
                                    <div className="px-3 py-2.5 border-t border-white/[0.04] max-h-32 overflow-auto">
                                        <p className="text-white/50 text-xs leading-relaxed">{result.originalPrompt}</p>
                                    </div>
                                )}
                            </div>
                        )}
                            ...
                            <div className="rounded-xl border border-white/[0.04] overflow-hidden">
                                ...
                                >
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="text-[10px] text-white/40 uppercase tracking-wider font-medium">Why Better</span>
                                    </div>
                                    <ChevronDown className={`w-3.5 h-3.5 text-white/30 transition-transform ${showExplanation ? 'rotate-180' : ''}`} />
                                </button>
                                {showExplanation && (
                                    <div className="px-3 py-2.5 border-t border-white/[0.04] max-h-48 overflow-auto">
